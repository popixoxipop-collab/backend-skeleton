...
| T08-02 transport | implemented in branch | `bskel.native-language/1`, fatal UTF-8, LF/CRLF, request ID, fact validation, byte/route/diagnostic budgets |
...
| T08-06 platform packaging | partial | npm package inclusion has a dedicated regression; compiler/toolchain backend availability is not yet certified |

## Verified on EOE

At branch head before this status-only update:

- focused native-server suite: 32/32 PASS;
