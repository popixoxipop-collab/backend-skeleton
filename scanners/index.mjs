...
export function runScan({ repoRoot, terms, includeDb = false }) {
	...
	let modules;
	let confidence;
	let pathPrefixSignals;

	if (javaResult) {
		adapter = 'java-spring';
		modules = javaResult.modules;
		confidence = 'high';
		pathPrefixSignals = javaResult.pathPrefixSignals;
	} else {
		const generic = scanGenericGrep(repoRoot);
		adapter = 'generic-grep';
		modules = generic.modules;
		confidence = 'low';
		// A1 §7's three signals (WebMvcConfigurer, server.servlet.context-path, springdoc) are all
		// Spring-specific concepts -- nothing to detect for a non-Spring repo.
		pathPrefixSignals = [];
	}
	...
	return {
		...
		adapter,
		confidence,
		// A1 §7: previously claimed "(no committed openapi spec found)" unconditionally -- a scan
		// that never actually checked for one. Describes what this scan does/doesn't verify, not an
		// unverified claim about the repo.
		api_surface_source: 'source-annotations only (this scan does not check for a committed ' +
