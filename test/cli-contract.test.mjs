...
for (const name of ['scan disposition', 'feature init', 'contract emit', 'contract waive', 'contract validate', 'contract tool-schema', 'handles plan', 'handles emit', 'verify']) {
...
test('every command in COMMANDS is exercised by the default-value snapshot or the required-field test above', () => {
	const covered = new Set([...Object.keys(EXPECTED_DEFAULTS), 'scan disposition', 'feature init', 'contract emit', 'contract waive', 'contract validate', 'contract tool-schema', 'handles plan', 'handles emit', 'verify']);
