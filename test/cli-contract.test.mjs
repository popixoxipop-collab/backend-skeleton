...
for (const name of ['scan disposition', 'scan explain', 'feature init', 'feature rename', 'contract emit', 'contract export', 'contract history', 'contract waive', 'contract validate', 'contract tool-schema', 'gate export', 'handles plan', 'handles emit', 'handles patch approve', 'handles audit', 'observe emit', 'observe import', 'new', 'verify']) {
...
test('every command in COMMANDS is exercised by the default-value snapshot or the required-field test above', () => {
	const covered = new Set([...Object.keys(EXPECTED_DEFAULTS), 'scan disposition', 'scan explain', 'feature init', 'feature rename', 'contract emit', 'contract export', 'contract history', 'contract waive', 'contract validate', 'contract tool-schema', 'gate export', 'handles plan', 'handles emit', 'handles patch approve', 'handles audit', 'observe emit', 'observe import', 'new', 'verify']);
