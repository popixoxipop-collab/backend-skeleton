...
const EXPECTED_DEFAULTS = {
	...
	'webgame scan': { _: [], json: false, quiet: false, help: false },
	...
	serve: { _: [], port: '4747', host: '127.0.0.1', 'database-url-env': null, schema: 'public', 'sign-key': null, 'require-sign-key': false, json: false, quiet: false, help: false },
...
for (const name of ['scan disposition', 'scan explain', 'scan repair', 'scan cross-feature-check', 'scan cross-feature-waive', 'scan cross-feature-unwaive', 'feature init', 'feature rename', 'webgame contract emit', 'webgame contract verify', 'contract emit', 'contract export', 'contract export-csv', 'contract history', 'contract waive', 'contract unwaive', 'contract validate', 'contract tool-schema', 'gate export', 'handles plan', 'handles emit', 'handles patch approve', 'handles patch unapprove', 'handles audit', 'observe emit', 'observe import', 'new', 'pattern list', 'pattern show', 'pattern suggest', 'verify', 'dependency declare', 'dependency remove', 'dependency list', 'rules check', 'rules list', 'rules explain', 'rules emit', 'patch propose', 'patch approve', 'patch apply', 'patch rollback', 'patch list', 'attest keygen', 'attest verify', 'impact accept', 'impact disposition', 'impact ack', 'impact export']) {
...
test('every command in COMMANDS is exercised by the default-value snapshot or the required-field test above', () => {
	const covered = new Set([...Object.keys(EXPECTED_DEFAULTS), 'scan disposition', 'scan explain', 'scan repair', 'scan cross-feature-check', 'scan cross-feature-waive', 'scan cross-feature-unwaive', 'feature init', 'feature rename', 'webgame contract emit', 'contract emit', 'contract export', 'contract export-csv', 'contract history', 'contract waive', 'contract unwaive', 'contract validate', 'contract tool-schema', 'gate export', 'handles plan', 'handles emit', 'handles patch approve', 'handles patch unapprove', 'handles audit', 'observe emit', 'observe import', 'new', 'pattern list', 'pattern show', 'pattern suggest', 'verify', 'dependency declare', 'dependency remove', 'dependency list', 'rules check', 'rules list', 'rules explain', 'rules emit', 'patch propose', 'patch approve', 'patch apply', 'patch rollback', 'patch list', 'attest keygen', 'attest verify', 'impact accept', 'impact disposition', 'impact ack', 'impact export']);
