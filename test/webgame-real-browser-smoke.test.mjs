...
test('owned fixture passes real Chromium input and probe verification when local browser tooling is available', async (t) => {
  let playwright;
  try {
  ...
  const raw = {
    ...
    project_id: 'owned-browser-smoke',
    source: { trust: 'owned', root: 'fixtures/webgame-runtime/owned-singleplayer' },
  ...
  const plan = buildWebgameExecutionPlan(parsed, {
    approval: {
      approval_id: 'local-browser-smoke',
      allow_execute: true,
