...
function live({provider='postgres-introspection',columns=['tenant_id','id'],types={tenant_id:'uuid',id:'uuid'}}={}) {
	return createPersistenceIr({ provider, source_kind:'live', entities:[{
...
test('live verification preserves ordered composite PK and recognizes compatible UUID source/live types', () => {
	...
	const verified = verifyResourceBindingsAgainstObserved({ binding_result: bound, observed: live(), expected_live_provider:'postgres-introspection' });
	...
	assert.deepEqual(verified.bindings[0].verification,{source_kind:'live',provider:'verified',expected_provider:'postgres-introspection',observed_provider:'postgres-introspection',table:'verified',primary_key:'verified',key_type:'source-only'});
});
...
test('unknown source key type adopts single live DB key type after exact PK verification',()=>{
	...
	const verified=verifyResourceBindingsAgainstObserved({binding_result:bound,observed:live({columns:['id'],types:{id:'uuid'}}),expected_live_provider:'postgres-introspection'});
...
test('source/live key type contradiction blocks verified read',()=>{
	...
	const verified=verifyResourceBindingsAgainstObserved({binding_result:bound,observed:live({columns:['id'],types:{id:'bigint'}}),expected_live_provider:'postgres-introspection'});
...
test('live verification refuses reordered composite PK', () => {
	const bound = composeResourceBindings({ resources: [{ id: 'users', entity_ref: entityId('test','User','users') }], persistence: persistence() });
	const verified = verifyResourceBindingsAgainstObserved({ binding_result: bound, observed: live({columns:['id','tenant_id']}), expected_live_provider:'postgres-introspection' });
	assert.equal(verified.bindings[0].capabilities.verified_read_by_primary_key, false);
...
test('non-live evidence cannot be used to runtime-verify a binding', () => {
	const bound = composeResourceBindings({ resources: [{ id: 'users', entity_ref: entityId('test','User','users') }], persistence: persistence() });
	assert.throws(() => verifyResourceBindingsAgainstObserved({ binding_result: bound, observed: persistence(), expected_live_provider:'postgres-introspection' }), /source_kind=live/);
});
...
test('live verification defaults an unspecified source schema to the observed live schema',()=>{
	...
	const verified=verifyResourceBindingsAgainstObserved({binding_result:bound,observed,expected_live_provider:'postgres-introspection'});
