...
test('actual failure remains failure and cannot be hidden as missing', () => {
  ...
  target.outcome = 'fail';
  target.evidence_refs = ['artifact:process-tree-leak'];
  const report = evaluateAdversarialResults(SPEC, results);
  assert.equal(report.ready, false);
...
test('unknown/duplicate cases and unknown fields fail closed', () => {
  ...
  results.push({ id: 'TRUST-NET-99', outcome: 'pass', observer_kind: 'network-probe', evidence_refs: ['x'] });
...
test('declaration-only evaluation can be requested separately without changing runner gate', () => {
  const results = SPEC.cases
    ...
    .map((x) => ({ id: x.id, outcome: 'pass', observer_kind: 'validator', evidence_refs: [`test:${x.id}`] }));
  const report = evaluateAdversarialResults(SPEC, results, { requiredLayers: ['declaration'] });
