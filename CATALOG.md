...
- **Status note (2026-08-16):** the process.exit() pipe-truncation audit (`3b5106a`) fixed the specific truncation bug in some exit paths but did not touch the broader "unknown flags silently become positional args" / "errors not reliably JSON" issues this item is actually about — still open.
