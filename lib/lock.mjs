...
async function acquireAsyncOrThrow(repoRoot, lockName, timeoutMs, staleAfterMs) {
	const dir = locksDir(repoRoot);
	fs.mkdirSync(dir, { recursive: true });
	const lockPath = path.join(dir, `${lockName}.lock`);
...
export async function withLockAsync(repoRoot, lockName, fn, { timeoutMs = DEFAULT_TIMEOUT_MS, staleAfterMs = DEFAULT_STALE_AFTER_MS } = {}) {
	const lockPath = await acquireAsyncOrThrow(repoRoot, lockName, timeoutMs, staleAfterMs);
	const cleanup = () => {
