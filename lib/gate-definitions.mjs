...
function gameplayFiles(root, manifests) {
	const files = new Set();
	const add = (relative) => { if (relative && fs.existsSync(path.join(root, relative))) files.add(relative); };
	const walk = (relative) => {
	...
	for (const manifest of manifests) {
		add(manifest.compiler.script); add(manifest.compiler.world_anchors); add(manifest.compiler.output); add(manifest.runtime.project_file);
		for (const step of [...manifest.runtime.emit_steps, ...manifest.runtime.verify_steps]) {
			add(step.script); for (const arg of step.args) add(arg); add(step.result_file); add(path.join('.sbf', 'gameplay-receipts', manifest.loop_id, `${step.id}.json`));
			for (const target of [...(step.writes ?? []), ...(step.reads ?? [])]) {
				if (target.kind === 'file') add(target.path);
				else {
					const stem = target.path.replace(/^\/Game\//, 'Content/');
					add(`${stem}.uasset`); add(`${stem}.umap`);
					if (target.path.endsWith('/Lvl_ThirdPerson')) walk(path.join('Content', '__ExternalActors__', 'ThirdPerson', 'Lvl_ThirdPerson'));
				}
			}
		}
	}
...
export function gameplayReceiptIntegrityInputs(root, manifests) {
	const inputs = {};
	for (const manifest of manifests) {
		for (const step of [...manifest.runtime.emit_steps, ...manifest.runtime.verify_steps]) {
			const receiptFile = path.join(root, '.sbf', 'gameplay-receipts', manifest.loop_id, `${step.id}.json`);
			const externalFile = path.join(root, ...step.result_file.split('/'));
			const record = readJsonIfExists(receiptFile);
			const actualHash = sha256File(externalFile);
			const expectedHash = record?.external_receipt_hash ?? null;
			const ok = record?.schema === 'sbf.gameplay-receipt/1'
				&& record.loop_id === manifest.loop_id
				&& record.step_id === step.id
				&& typeof expectedHash === 'string'
				&& expectedHash === actualHash;
			inputs[`gameplay:receipt-integrity:${manifest.loop_id}:${step.id}`] = ok
				? `ok:${actualHash}`
				: `invalid:expected=${expectedHash ?? 'missing'}:actual=${actualHash ?? 'missing'}`;
		}
	}
	return inputs;
}
...
export const GATE_DEFINITIONS = Object.freeze({
	...
	gameplay: {
		...
		recompute: (root) => {
			...
			const manifests = readGameRuntimeManifests(root, { contracts: contracts.contracts });
			const inputs = {};
			const runtimeLoopIds = new Set(manifests.manifests.map((manifest) => manifest.loop_id));
			const runtimeContractFiles = contracts.contracts.filter((contract) => runtimeLoopIds.has(contract.loop_id)).map((contract) => contract.file);
			for (const file of [...runtimeContractFiles, ...manifests.files_read, ...gameplayFiles(root, manifests.manifests)]) inputs[`gameplay:${file}`] = sha256File(path.join(root, file));
			Object.assign(inputs, gameplayReceiptIntegrityInputs(root, manifests.manifests));
			return inputs;
		},
	},
});
...
export const GATE_NAMES = Object.freeze(['preflight', 'scan', 'cross_feature', 'contract', 'dependencies', 'impact', 'rules', 'handles', 'stack', 'patch_transactions', 'conformance', 'gameplay']);
