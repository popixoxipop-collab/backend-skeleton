...
export const GATE_DEFINITIONS = Object.freeze({
	...
	gameplay: {
		...
		recompute: (root) => {
			...
			const inputs = {};
			for (const file of [...contracts.files_read, ...manifests.files_read]) inputs[`gameplay:${file}`] = sha256File(path.join(root, file));
			for (const manifest of manifests.manifests) inputs[`gameplay:${manifest.compiler.output}`] = sha256File(path.join(root, manifest.compiler.output));
			return inputs;
		},
	},
});
