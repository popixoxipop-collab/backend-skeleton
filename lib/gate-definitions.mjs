...
export const GATE_DEFINITIONS = Object.freeze({
	...
	gameplay: {
		...
		recompute: (root) => {
			...
			const inputs = {};
			for (const file of [...contracts.files_read, ...manifests.files_read, ...gameplayFiles(root, manifests.manifests)]) inputs[`gameplay:${file}`] = sha256File(path.join(root, file));
			return inputs;
		},
	},
});
