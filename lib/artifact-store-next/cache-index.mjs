...
export function createCacheIndex(repoRoot, { relativePath = '.sbf/cache-next/index.json', lockName = 't21-cache-index' } = {}) {
	...
	function list() {
		const state = load();
		return Object.entries(state.entries).sort(([a], [b]) => a.localeCompare(b)).map(([cache_key, entry]) => ({ cache_key, ...clone(entry) }));
	}
	...
	return { schema: SCHEMA, path: statePath, get, set, remove, list, reachableArtifactRefs };
}
