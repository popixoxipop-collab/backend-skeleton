...
const ECHO_KEYS = new Set(['contract', 'permission', 'artifact_policy']);
...
export function validateTrustEvidenceEcho(requirements, echo) {
  const errors = [];
...
export function assertTrustEvidenceEcho(requirements, echo) {
  const result = validateTrustEvidenceEcho(requirements, echo);
