...
const MAX_ALLOW_ENTRIES = 1024;
const MAX_REVOKED_ENTRIES = 1024;
...
function normalizeAllow(errors, value) {
  ...
  if (value.length > MAX_ALLOW_ENTRIES) err(errors, 'TOO_MANY_TRUST_ALLOW_ENTRIES', 'allow', `must contain at most ${MAX_ALLOW_ENTRIES} entries`);
  const byKey = new Map();
  for (let i = 0; i < Math.min(value.length, MAX_ALLOW_ENTRIES); i += 1) {
    const item = value[i];
    const path = `allow[${i}]`;
    if (!plain(item)) {
...
function normalizeRevoked(errors, value) {
  ...
  if (value.length > MAX_REVOKED_ENTRIES) err(errors, 'TOO_MANY_REVOCATIONS', 'revoked', `must contain at most ${MAX_REVOKED_ENTRIES} entries`);
  const byDigest = new Map();
  for (let i = 0; i < Math.min(value.length, MAX_REVOKED_ENTRIES); i += 1) {
    const item = value[i];
    ...
    if (!validSha(item.sha256)) err(errors, 'INVALID_ARTIFACT_SHA256', `${path}.sha256`, 'must be a lowercase 64-hex SHA-256');
    const reasonValid = typeof item.reason === 'string' &&
      item.reason.trim().length >= 8 && item.reason.length <= 500 &&
      !/[\u0000-\u001f\u007f]/.test(item.reason);
    if (!reasonValid) {
      err(errors, 'INVALID_REVOCATION_REASON', `${path}.reason`, 'must be 8..500 non-blank printable characters with no control characters');
    }
    if (validSha(item.sha256) && reasonValid) {
...
export function assertNoArtifactTrustExpansion(beforeInput, afterInput) {
  const delta = diffArtifactTrustPolicies(beforeInput, afterInput);
  if (!delta.expanded) return delta;
  const error = new Error('artifact trust update expands privileges and requires an external approval decision');
  error.code = 'ARTIFACT_TRUST_EXPANSION_REQUIRES_APPROVAL';
  error.delta = delta;
  throw error;
}
