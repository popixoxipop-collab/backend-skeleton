...
function normalizeNetwork(errors, value) {
  ...
  for (let i = 0; i < rawAllow.length; i += 1) {
    ...
    if (typeof rule.host !== 'string' || !HOST.test(rule.host) || rule.host.includes('*') || rule.host.includes('://')) {
...
function normalizeProcess(errors, value) {
  ...
  if (!Array.isArray(raw)) push(errors, 'INVALID_PERMISSION_FIELD', 'process.executables', 'must be an array');
    ...
    if (typeof executable !== 'string' || !EXECUTABLE.test(executable) || executable.includes('/') || executable.includes('\\')) {
...
export function compilePermissionPolicy(input) {
  ...
  return Object.freeze({
    ...
    canExecute(executable) {
      return manifest.process.mode === 'argv-allowlist' && typeof executable === 'string' && EXECUTABLE.test(executable) && manifest.process.executables.includes(executable);
    },
