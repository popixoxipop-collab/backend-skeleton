...
export const PERMISSION_MANIFEST_DIGEST_FORMAT = 'bskel.trust-permissions-json/1';

const TOP_LEVEL_KEYS = new Set([
  'schema', 'read_roots', 'write_roots', 'network', 'listen', 'process',
  'environment', 'secret_refs', 'devices', 'limits',
]);
...
const DEVICE_KEYS = new Set(['mode', 'allow']);
const DEVICE_CLASSES = new Set(['gpu', 'display', 'camera', 'audio', 'usb', 'accelerator']);
const LIMIT_KEYS = new Set(['wall_ms', 'cpu_ms', 'memory_bytes', 'pids', 'stdout_bytes', 'stderr_bytes', 'scratch_bytes']);
...
const MAX_ROOTS = 256;
const MAX_NETWORK_RULES = 256;
const MAX_PORTS_PER_RULE = 256;
const MAX_EXECUTABLES = 128;
const MAX_ENV_NAMES = 128;
const MAX_SECRET_REFS = 128;
...
function portableRoot(value) {
  if (typeof value !== 'string' || value.length === 0 || value.length > MAX_ROOT_LENGTH || /[\u0000-\u001f\u007f]/.test(value) || value.includes('\\')) return null;
...
function normalizeRoots(errors, value, field) {
  ...
  if (value.length > MAX_ROOTS) push(errors, 'TOO_MANY_PERMISSION_ROOTS', field, `must contain at most ${MAX_ROOTS} roots`);
  const out = [];
  for (let i = 0; i < Math.min(value.length, MAX_ROOTS); i += 1) {
...
function normalizeNetwork(errors, value) {
  ...
  if (rawAllow.length > MAX_NETWORK_RULES) push(errors, 'TOO_MANY_NETWORK_RULES', 'network.allow', `must contain at most ${MAX_NETWORK_RULES} rules`);
  const allow = [];
  for (let i = 0; i < Math.min(rawAllow.length, MAX_NETWORK_RULES); i += 1) {
    ...
    if (!Array.isArray(ports) || ports.length === 0 || ports.length > MAX_PORTS_PER_RULE || ports.some((p) => !Number.isInteger(p) || p < 1 || p > 65535)) {
      push(errors, 'INVALID_NETWORK_PORTS', `network.allow[${i}].ports`, `must contain 1..${MAX_PORTS_PER_RULE} integer ports in 1..65535`);
      continue;
    }
...
function normalizeListen(errors, value) {
  if (value === undefined) return { mode: 'deny', allow: [] };
  if (!isPlainObject(value)) {
    push(errors, 'INVALID_PERMISSION_FIELD', 'listen', 'must be an object');
    return { mode: 'deny', allow: [] };
  }
  rejectUnknownKeys(errors, value, NETWORK_KEYS, 'listen');
  const mode = value.mode ?? 'deny';
  if (!['deny', 'allowlist'].includes(mode)) push(errors, 'INVALID_LISTEN_MODE', 'listen.mode', 'must be deny or allowlist');
  const rawAllow = value.allow ?? [];
  if (!Array.isArray(rawAllow)) {
    push(errors, 'INVALID_PERMISSION_FIELD', 'listen.allow', 'must be an array');
    return { mode: mode === 'allowlist' ? 'allowlist' : 'deny', allow: [] };
  }
  if (rawAllow.length > MAX_NETWORK_RULES) push(errors, 'TOO_MANY_LISTEN_RULES', 'listen.allow', `must contain at most ${MAX_NETWORK_RULES} rules`);
  const allowedHosts = new Set(['127.0.0.1', '[::1]', 'localhost']);
  const allow = [];
  for (let i = 0; i < Math.min(rawAllow.length, MAX_NETWORK_RULES); i += 1) {
    const rule = rawAllow[i];
    if (!isPlainObject(rule)) {
      push(errors, 'INVALID_LISTEN_RULE', `listen.allow[${i}]`, 'must be an object');
      continue;
    }
    rejectUnknownKeys(errors, rule, NETWORK_RULE_KEYS, `listen.allow[${i}]`);
    const host = typeof rule.host === 'string' ? rule.host.toLowerCase() : '';
    if (!allowedHosts.has(host)) {
      push(errors, 'INVALID_LISTEN_HOST', `listen.allow[${i}].host`, 'only explicit loopback bind hosts 127.0.0.1, [::1], or localhost are allowed in this trust revision');
      continue;
    }
    const ports = rule.ports ?? [];
    if (!Array.isArray(ports) || ports.length === 0 || ports.length > MAX_PORTS_PER_RULE || ports.some((p) => !Number.isInteger(p) || p < 1 || p > 65535)) {
      push(errors, 'INVALID_LISTEN_PORTS', `listen.allow[${i}].ports`, `must contain 1..${MAX_PORTS_PER_RULE} integer ports in 1..65535`);
      continue;
    }
    allow.push({ host, ports: [...new Set(ports)].sort((a,b)=>a-b) });
  }
  const mergedByHost = new Map();
  for (const rule of allow) {
    const ports = mergedByHost.get(rule.host) ?? new Set();
  ...
  const normalizedAllow = [...mergedByHost.entries()]
    .map(([host, ports]) => ({ host, ports: [...ports].sort((a,b)=>a-b) }))
    .sort((a,b)=>a.host.localeCompare(b.host));
  if (mode === 'deny' && normalizedAllow.length > 0) push(errors, 'DENY_WITH_LISTEN_ALLOWLIST', 'listen.allow', 'deny mode cannot carry listen grants');
  if (mode === 'allowlist' && normalizedAllow.length === 0) push(errors, 'EMPTY_LISTEN_ALLOWLIST', 'listen.allow', 'allowlist mode requires at least one loopback endpoint');
  return { mode: mode === 'allowlist' ? 'allowlist' : 'deny', allow: normalizedAllow };
}
...
function normalizeProcess(errors, value) {
  ...
  if (!Array.isArray(raw)) push(errors, 'INVALID_PERMISSION_FIELD', 'process.executables', 'must be an array');
    ...
    if (raw.length > MAX_EXECUTABLES) push(errors, 'TOO_MANY_EXECUTABLES', 'process.executables', `must contain at most ${MAX_EXECUTABLES} entries`);
    for (let i = 0; i < Math.min(raw.length, MAX_EXECUTABLES); i += 1) {
...
function normalizeEnvironment(errors, value) {
  ...
  if (raw.length > MAX_ENV_NAMES) push(errors, 'TOO_MANY_ENV_NAMES', 'environment.allow', `must contain at most ${MAX_ENV_NAMES} entries`);
  const allow = [];
  for (let i = 0; i < Math.min(raw.length, MAX_ENV_NAMES); i += 1) {
...
function normalizeDevices(errors, value) {
  if (value === undefined) return { mode: 'deny', allow: [] };
  if (!isPlainObject(value)) {
    push(errors, 'INVALID_PERMISSION_FIELD', 'devices', 'must be an object');
    return { mode: 'deny', allow: [] };
  }
  rejectUnknownKeys(errors, value, DEVICE_KEYS, 'devices');
  const mode = value.mode ?? 'deny';
  if (!['deny', 'allowlist'].includes(mode)) push(errors, 'INVALID_DEVICE_MODE', 'devices.mode', 'must be deny or allowlist');
  const raw = value.allow ?? [];
  if (!Array.isArray(raw)) {
    push(errors, 'INVALID_PERMISSION_FIELD', 'devices.allow', 'must be an array');
    return { mode: mode === 'allowlist' ? 'allowlist' : 'deny', allow: [] };
  }
  if (raw.length > 32) push(errors, 'TOO_MANY_DEVICE_GRANTS', 'devices.allow', 'must contain at most 32 device classes');
  const allow = [];
  for (let i = 0; i < Math.min(raw.length, 32); i += 1) {
    if (!DEVICE_CLASSES.has(raw[i])) push(errors, 'INVALID_DEVICE_CLASS', `devices.allow[${i}]`, 'unsupported device class');
    else allow.push(raw[i]);
  }
  const normalized = [...new Set(allow)].sort();
  if (mode === 'deny' && normalized.length > 0) push(errors, 'DENY_WITH_DEVICE_ALLOWLIST', 'devices.allow', 'deny mode cannot carry device grants');
  if (mode === 'allowlist' && normalized.length === 0) push(errors, 'EMPTY_DEVICE_ALLOWLIST', 'devices.allow', 'allowlist mode requires at least one device class');
  return { mode: mode === 'allowlist' ? 'allowlist' : 'deny', allow: normalized };
}
...
function normalizeSecretRefs(errors, value) {
  ...
  if (value.length > MAX_SECRET_REFS) push(errors, 'TOO_MANY_SECRET_REFS', 'secret_refs', `must contain at most ${MAX_SECRET_REFS} entries`);
  const refs = [];
  for (let i = 0; i < Math.min(value.length, MAX_SECRET_REFS); i += 1) {
    const ref = value[i];
...
function normalizeLimits(errors, value) {
  const defaults = { wall_ms: 30_000, cpu_ms: 30_000, memory_bytes: 1_073_741_824, pids: 32, stdout_bytes: 1_048_576, stderr_bytes: 1_048_576, scratch_bytes: 268_435_456 };
...
export function validatePermissionManifest(input) {
  ...
  const value = {
    ...
    listen: normalizeListen(errors, input.listen),
    ...
    devices: normalizeDevices(errors, input.devices),
...
function canonicalPermissionObject(manifest) {
  return {
    schema: manifest.schema,
    read_roots: [...manifest.read_roots],
    write_roots: [...manifest.write_roots],
    network: {
      mode: manifest.network.mode,
      allow: manifest.network.allow.map((x) => ({ host: x.host, ports: [...x.ports] })),
    },
    listen: {
      mode: manifest.listen.mode,
      allow: manifest.listen.allow.map((x) => ({ host: x.host, ports: [...x.ports] })),
    },
    process: {
      mode: manifest.process.mode,
      executables: [...manifest.process.executables],
      max_children: manifest.process.max_children,
    },
    environment: { allow: [...manifest.environment.allow] },
    secret_refs: [...manifest.secret_refs],
    devices: { mode: manifest.devices.mode, allow: [...manifest.devices.allow] },
    limits: {
      wall_ms: manifest.limits.wall_ms,
      cpu_ms: manifest.limits.cpu_ms,
      ...
      scratch_bytes: manifest.limits.scratch_bytes,
    },
  };
}

export function serializePermissionManifest(input) {
  const result = validatePermissionManifest(input);
  if (!result.ok) {
    const error = new TypeError('cannot serialize an invalid permission manifest');
    error.code = 'INVALID_PERMISSION_MANIFEST';
    error.details = result.errors;
    throw error;
  }
  return `${JSON.stringify(canonicalPermissionObject(result.value))}\n`;
}

export function permissionManifestDigest(input) {
  const bytes = Buffer.from(`${PERMISSION_MANIFEST_DIGEST_FORMAT}\n${serializePermissionManifest(input)}`, 'utf8');
...
export function compilePermissionPolicy(input) {
  ...
  return Object.freeze({
    ...
    canListen(host, port) {
      if (manifest.listen.mode !== 'allowlist' || typeof host !== 'string' || !Number.isInteger(port)) return false;
      const normalized = host.toLowerCase();
      return manifest.listen.allow.some((rule) => rule.host === normalized && rule.ports.includes(port));
    },
    ...
    canUseDevice(deviceClass) { return manifest.devices.mode === 'allowlist' && typeof deviceClass === 'string' && manifest.devices.allow.includes(deviceClass); },
  });
}
...
export function diffPermissionManifests(beforeInput, afterInput) {
  ...
  expansions.push(...setDelta(endpointSet(before.listen), endpointSet(after.listen), 'network.listen'));
  reductions.push(...setDelta(endpointSet(after.listen), endpointSet(before.listen), 'network.listen'));
  ...
  expansions.push(...setDelta(new Set(before.devices.allow), new Set(after.devices.allow), 'device.use'));
  reductions.push(...setDelta(new Set(after.devices.allow), new Set(before.devices.allow), 'device.use'));
  ...
  for (const key of ['wall_ms', 'cpu_ms', 'memory_bytes', 'pids', 'stdout_bytes', 'stderr_bytes', 'scratch_bytes']) {
...
export function assertNoPermissionExpansion(beforeInput, afterInput) {
  const delta = diffPermissionManifests(beforeInput, afterInput);
  if (!delta.expanded) return delta;
  const error = new Error('permission update expands privileges and requires an external approval decision');
  error.code = 'PERMISSION_EXPANSION_REQUIRES_APPROVAL';
  error.delta = delta;
  throw error;
}
