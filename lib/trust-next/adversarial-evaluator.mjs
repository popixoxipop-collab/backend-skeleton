...
function normalizeEvidenceRefs(value, field) {
  if (!Array.isArray(value) || value.length > 64) fail('ADVERSARIAL_RESULT_EVIDENCE_INVALID', `${field} must be an array with at most 64 refs`);
  const out = [];
  for (let i = 0; i < value.length; i += 1) {
    const ref = value[i];
    if (typeof ref !== 'string' || !EVIDENCE_REF.test(ref) || /[\u0000-\u001f\u007f]/.test(ref)) {
      fail('ADVERSARIAL_RESULT_EVIDENCE_INVALID', `${field}[${i}] must be a bounded opaque evidence reference`);
    }
    out.push(ref);
  }
