...
const CLASSES = new Set(['network-command', 'database-read', 'database-write', 'local-server']);
...
export function buildNetworkCommandRequirements({
  commandId,
  connectAllow,
  secretRefs = [],
  artifactTrustPolicy,
  ...
  if (typeof commandId !== 'string' || !ID.test(commandId)) {
    fail('NETWORK_COMMAND_ID_INVALID', 'commandId must be a stable lowercase identifier');
  }
  if (!Array.isArray(connectAllow) || connectAllow.length < 1 || connectAllow.length > 64) {
    fail('NETWORK_COMMAND_ENDPOINTS_INVALID', 'connectAllow must contain 1..64 exact endpoints');
  }
  const outbound = connectAllow.map((x, i) => exactEndpoint(x, `connectAllow[${i}]`));
  if (!Array.isArray(secretRefs) || secretRefs.length > 64 ||
      secretRefs.some((x) => typeof x !== 'string' || !SECRET_REF.test(x))) {
    fail('NETWORK_COMMAND_SECRET_REFS_INVALID', 'secretRefs must be at most 64 opaque T20 secret identifiers');
  }
  const trustPolicy = validateTrustPolicy(artifactTrustPolicy);
  const permission = buildPermission({
    schema: 'bskel.trust-permissions/1',
    read_roots: [],
    write_roots: [],
    network: { mode: 'allowlist', allow: outbound.map((x) => ({ host: x.host, ports: [x.port] })) },
    listen: { mode: 'deny', allow: [] },
    process: { mode: 'deny', executables: [], max_children: 0 },
    environment: { allow: [] },
    secret_refs: [...secretRefs],
    devices: { mode: 'deny', allow: [] },
    limits: { ...limits },
  }, 'NETWORK_COMMAND_PERMISSION_REJECTED');

  return Object.freeze({
    contract: SERVICE_ACCESS_REQUIREMENTS_CONTRACT,
    service_class: 'network-command',
    ...
    runtime_binding_required: true,
    evidence_required: true,
    note: 'This profile is for first-party in-process outbound requests. git/gh/curl or other subprocess networking must use a separately trusted first-party helper/process profile.',
  });
}
