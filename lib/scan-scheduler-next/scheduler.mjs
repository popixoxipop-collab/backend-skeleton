...
function normalizeTasks(tasks) {
	...
	for (const task of tasks) {
		...
		byId.set(task.id, { ...task, dependsOn: [...new Set(dependsOn)].sort(), estimatedBytes });
	}
...
export async function runTaskGraph(tasks, worker, {
	...
	const markBlockedDependencies = () => {
		let changed = false;
		for (const task of [...byId.values()].sort((a, b) => a.id.localeCompare(b.id))) {
	...
	while ([...state.values()].some((x) => !terminal.has(x.status))) {
		...
		for (const task of ready) {
			...
			if (bytesInFlight + task.estimatedBytes > maxEstimatedBytesInFlight) continue;
	...
	const results = [...state.values()].sort((a, b) => a.id.localeCompare(b.id));
	return {
