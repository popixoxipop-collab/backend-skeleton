...
export function evaluatePerformanceGate({
	...
	for (const id of ids) {
		...
		const bCold = finitePositive(`baseline[${id}].cold_p95_ms`, b.cold_p95_ms);
		const cCold = finitePositive(`candidate[${id}].cold_p95_ms`, c.cold_p95_ms);
		const bRss = finitePositive(`baseline[${id}].peak_rss_bytes`, b.peak_rss_bytes);
		const cRss = finitePositive(`candidate[${id}].peak_rss_bytes`, c.peak_rss_bytes);
		const hit = finitePositive(`candidate[${id}].cache_hit_p95_ms`, c.cache_hit_p95_ms);
