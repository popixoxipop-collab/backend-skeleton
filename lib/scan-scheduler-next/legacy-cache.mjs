...
export async function runLegacyScanCached({
	...
	if (cacheNamespace !== null) requireDigest('cacheNamespace', cacheNamespace);
	if (policyFingerprint !== null) requireDigest('policyFingerprint', policyFingerprint);
	...
	const cacheKey = buildScanCacheKey({
		...
		policyDigest: digestJson({
			...
			external_policy_fingerprint: policyFingerprint,
			db: 'disabled',
