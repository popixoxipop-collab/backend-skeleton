...
function detectPortablePathIssues(entries) {
	const byCase = new Map();
	const byNfc = new Map();
	for (const entry of entries) {
		const caseKey = entry.path.toLowerCase();
		const nfcKey = entry.path.normalize('NFC');
		if (!byCase.has(caseKey)) byCase.set(caseKey, []);
		if (!byNfc.has(nfcKey)) byNfc.set(nfcKey, []);
		byCase.get(caseKey).push(entry.path);
		byNfc.get(nfcKey).push(entry.path);
	}
	const issues = [];
	for (const paths of byCase.values()) if (new Set(paths).size > 1) issues.push({ kind: 'case-fold-collision', paths: [...paths].sort(compareText) });
	for (const paths of byNfc.values()) if (new Set(paths).size > 1) issues.push({ kind: 'unicode-nfc-collision', paths: [...paths].sort(compareText) });
	return issues.sort((a, b) => compareText(`${a.kind}:${a.paths.join('\0')}`, `${b.kind}:${b.paths.join('\0')}`));
}

export function portablePathIssuesForPaths(paths) {
	if (!Array.isArray(paths) || paths.some((value) => typeof value !== 'string')) {
		throw new TypeError('paths must be a string array');
	}
	return detectPortablePathIssues(paths.map((path) => ({ path })));
}
...
function hashFile(filePath, { chunkBytes = 1024 * 1024 } = {}) {
...
export function buildFileIndex(repoRoot, {
	...
	while (stack.length > 0) {
		...
		const children = fs.readdirSync(dir, { withFileTypes: true }).sort((a, b) => compareText(a.name, b.name));
	...
	const portablePathIssues = detectPortablePathIssues(entries);
	return {
		...
		portable_path_issues: portablePathIssues,
	};
}
