...
// `scripts/preflight-base-ref.sh` defines STALE_BASE(11)/WRONG_DEFAULT(12)/DIRTY(13) itself (a
// standalone bash script, "reusable outside this skill" per its own header comment -- it cannot
// import this module). Documented here only so this table stays the one place a human looks up
// what a `bskel preflight` exit code means; the script's own literals are the actual source of
// truth for those three values and are not re-derived from here.
