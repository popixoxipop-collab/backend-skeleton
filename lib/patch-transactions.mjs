...
import { readJsonIfExists, writeFileAtomic, sha256File, sha256String } from './fsutil.mjs';
...
export function applyTransaction(root, featureId, transactionId, freshKindPlan) {
	return withLockSync(root, 'state', () => {
		const txn = loadTransaction(root, featureId, transactionId);
		if (!txn) throw new Error(`no patch transaction "${transactionId}" for feature "${featureId}"`);
		...
		txn.apply = { at: new Date().toISOString(), postimage_file_hash: sha256String(freshKindPlan.renderedContent) };
		txn.status = 'applied';
		return saveTransaction(root, featureId, txn);
	});
}

export function rollbackTransaction(root, featureId, transactionId, reason, { force = false } = {}) {
	if (!reason || !reason.trim()) {
	...
	return withLockSync(root, 'state', () => {
		...
		if (currentHash !== txn.apply.postimage_file_hash && !force) {
			throw new Error(
				`"${txn.target.file}" has changed since transaction "${transactionId}" applied -- rolling back would silently clobber that change; pass --force --reason if intentional`,
			);
		}
		const original = readBlob(root, featureId, txn.preimage.file_hash);
		fs.writeFileSync(targetAbs, original);
		txn.rollback = { reason, at: new Date().toISOString(), ...(force ? { forced: true } : {}) };
