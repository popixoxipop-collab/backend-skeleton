...
export function loadManifest(root) {
	...
	if (!parsed) return { schema: MANIFEST_SCHEMA, files: {}, enforceRegistry: false };
	...
	// O3 (D-handle-registry-enforcement): additive field, no schema bump -- a manifest written
	// before this existed simply has no key here, and `false` is exactly its correct historical
	// value (registry enforcement did not exist yet, so it was never "on").
	return { enforceRegistry: false, ...parsed };
}
...
// e2e regression in test/handles-ownership-cli.test.mjs). `enforceRegistry` (O3,
// D-handle-registry-enforcement) is repo-wide, singleton state -- the same "infra is one
// all-or-nothing unit" scope O2's own `files` entries already give `global/handle/*` -- so it is
// always written, never omitted, even when `false` (the default).
export function saveManifest(root, manifest) {
	...
	writeFileAtomic(manifestPath(root), `${JSON.stringify({ schema: manifest.schema, files: sortedFiles, enforceRegistry: manifest.enforceRegistry ?? false }, null, 2)}\n`);
}
