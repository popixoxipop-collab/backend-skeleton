...
export function computeDoctorChecks(root, { workflow = null } = {}) {
	...
	// rg: both scanner adapters use it directly for scan, and handles/emit.mjs's
	// detectBasePackage() shells out to it independently during `handles emit` -- required for
	// both, even though only java-spring.mjs's own call site lacks a try/catch (a separate, known,
	// out-of-scope issue surfaced via that adapter's own diagnostics()).
