...
function checkFreshness(def, record) {
	if (!def?.freshness) return null;
	const maxAgeMinutes = record.evidence?.freshness?.max_age_minutes ?? def.freshness.defaultMaxAgeMinutes;
...
export function requireGate(repoRoot, featureId, gateName, currentInputs) {
	...
	if (record.forced) {
		...
		// D-preflight-freshness (S3): also exempt from the TTL below -- a forced pass's own
		// expiry is S4's "expiry by time, commit, or next input change" territory, not this one.
