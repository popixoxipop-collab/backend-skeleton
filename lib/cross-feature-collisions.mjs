...
// against, in three tiers. `liveDbSchema` is whatever the CLI boundary already resolved (the SAME
// resolveDbSchemaOrExit() result `cmdScan` itself uses -- this function never opens a connection
// itself). Plane C is already schema-wide (every table in `--schema`, not filtered to any one
// feature), so there is nothing to MERGE across features' own snapshots -- locating one usable
// snapshot is enough; it already contains every table in that schema.
function resolveLiveTables(root, featureId, liveDbSchema) {
	...
	// stable, repeatable choice, not an arbitrary one.
