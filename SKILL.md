...
bskel contract export --feature <id> [--out <path>] [--json] [--allow-unprefixed] [--status-codes range|literal]
  ...
  #
  #    IT IS A LOSSY, NARROW PROJECTION OF ONE FEATURE, AND IT SAYS SO -- disclosed in
  #    `info.description` AND in a machine-readable `info.x-bskel-omitted` array. Nothing is ever
  ...
  #    (parameter `schema` resolved through the same `inlineSchema()` used for bodies). A8
  #    (D-openapi-per-status): per-status responses (the source document's own real status codes and
  #    descriptions, keyed verbatim -- additive to, never replacing, the 2xx/4xx/5xx unions above)
  #    and non-JSON request media types (e.g. `multipart/form-data`) are ALSO emitted under the same
  #    "copied, never reconstructed" rule. Where no source document was given, or it said nothing for
  #    that operation, the key is omitted, meaning "unspecified" -- never invented. `security: []` is
  #    copied when the SOURCE said `[]` (a real, positive claim of "no auth required" from the
  #    document itself); it is still never invented as a default. `x-bskel-omitted` reflects this
