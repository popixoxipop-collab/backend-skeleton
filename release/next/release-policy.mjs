...
export function observedBlockers(inventory) {
  ...
  for (const repo of inventory?.repositories ?? []) {
    ...
    if (v.status !== 'completed' || v.conclusion !== 'success' || !SHA40.test(v.reviewed_head_sha ?? '')) blockers.add('CURRENT_MAIN_CI_NOT_GREEN');
    if (v.mode === 'zero-delta-merge-tree-equivalence') {
      if (v.ahead_by !== 1 || v.behind_by !== 0 || v.file_delta_count !== 0) blockers.add('MAIN_TREE_NOT_EQUIVALENT');
    }
    if (v.direct_main_ci !== true) blockers.add('FINAL_MAIN_PUSH_CI_NOT_DIRECT');
  }
...
export function verifyCompatibilityInventory(inventory) {
  ...
  for (const repo of inventory?.repositories ?? []) {
    ...
    if (!['completed'].includes(v.status)) push(errors, 'CI_STATUS', { role: repo.role });
    if (v.conclusion !== 'success') push(errors, 'CI_CONCLUSION', { role: repo.role });
    if (v.mode === 'zero-delta-merge-tree-equivalence' && (v.ahead_by !== 1 || v.behind_by !== 0 || v.file_delta_count !== 0)) push(errors, 'TREE_EQUIVALENCE', { role: repo.role });
  }
...
export function verifyReleasePlan(plan, inventory) {
  ...
  const dynamic=observedBlockers(inventory);
  ...
  const accepted=prereqs.length>0 && prereqs.every((x)=>x.required_state===x.observed_state);
  if (plan.release_allowed === true && (!accepted || declared.size>0 || dynamic.length>0)) push(errors, 'PREMATURE_RELEASE');
