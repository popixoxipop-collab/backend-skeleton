...
The owned suite contains **28 Track A tests**. For validation only, a temporary root shim imported `test/webgame-scan/**` into the repository's existing `npm test`; that shim is removed in the final Track A diff to preserve parallel path ownership.
